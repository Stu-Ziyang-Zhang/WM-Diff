...
def data_preprocess(data_path_list):
    ...
    train_masks = train_masks/255 if np.max(train_masks) > 1 else train_masks
    train_FOVs = train_FOVs/255 if np.max(train_FOVs) > 1 else train_FOVs
...
def create_patch_idx(img_fovs, args):
    ...
    while count < args.N_patches:
        ...
        x_center = random.randint(0 + int(args.train_patch_width / 2), 
                                  img_w - int(args.train_patch_width / 2))
        y_center = random.randint(0 + int(args.train_patch_height / 2), 
                                  img_h - int(args.train_patch_height / 2))

        if args.inside_FOV == 'center' or args.inside_FOV == 'all':
            if not is_patch_inside_FOV(x_center, y_center, img_fovs[n, 0], 
                                       args.train_patch_height, args.train_patch_width, 
                                       mode=args.inside_FOV):
                continue
